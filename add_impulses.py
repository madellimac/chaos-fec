...
class add_impulses(Py_Module):
	...
	def add(self, ix_x, ix_y, delta_x, delta_y, r_in, r_out):
		r_out[0,:] = r_in[:]
	...
	def __init__(self, N):
		...
		self.create_codelet(t_add, lambda slf, lsk, fid: slf.add(lsk[s_ix_x], lsk[s_ix_y], lsk[s_delta_x], lsk[s_delta_y], lsk[s_r_in], lsk[s_r_out])) # create codelet